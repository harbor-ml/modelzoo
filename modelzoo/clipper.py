...
def ensemble(inp: Image, metadata):
    ...
    models = [
        "ImageNet Classification ResNet18",
        "ImageNet Classification ResNet50",
...
def mask(inp: Image, metadata):
    ...
    df = pd.DataFrame(
        {"rank": list(range(1, len(labels) + 1)), "category": labels}
    ).astype(str)
    return df
